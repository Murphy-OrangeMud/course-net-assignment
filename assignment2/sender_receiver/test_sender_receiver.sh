...
## DESCRIPTION
##    EN.601.414/614 test script for assignment 2.
##    Runs 5 different tests between each possible sender/receiver combination.

# Check correct number of arguments
if [[ $# -ne 2 ]]; then
  printf "USAGE: $0 [server port] [window size]\n"
  exit
fi
...
SENDER=/vagrant/assignment2/sender_receiver/sender.py
RECEIVER=/vagrant/assignment2/sender_receiver/receiver.py
...
# $1 = sender, $2 = receiver, $3 = port, $4 = print separator (no if 0, yes otherwise),
# $5 = print diff (no if 0, yes otherwise)
function test {
  $2 $3 3 > test_output.txt &
  SERVER_PID=$!
  sleep 0.2
  $1 127.0.0.1 $3 3 < test_message.txt >/dev/null
  EXIT_STATUS=$?
  sleep 0.2
  kill $SERVER_PID
...
#####################################################
# Tests, $1 = sender, $2 = receiver, $3 = port
#####################################################
...
if [[ -f $SENDER && -f $RECEIVER ]]; then
  all-tests "python $SENDER" "python $RECEIVER" $PORT
else
  printf "\n$SKIP_MESSAGE"
